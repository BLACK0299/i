...
async def join_channel(event):
    try:
        await Tepthon1(JoinChannelRequest("@ALhkoma_1"))
...
⚝ مرحـبًا بك فـي أوامـر سـورس الـحـكـومـه
    ...
    if sender.id == ownerhmd_id:
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قـنوات فـي الوقـت الحالي , قـم بـتجـميع النقـاط بطريقـة مختلفـة') != -1:
                await Tepthon1.send_message(event.chat_id, f"تم الانتهاء من التجميع | AL")
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhmd_id:
        await event.reply("جاري تجميع النقـاط")
        await event.edit("جاري تجميع النقـاط")
        joinu = await Tepthon1(JoinChannelRequest('ALhkoma_1'))
        ...
        await Tepthon1.send_message(event.chat_id, "تم الانتهاء من التجميع | AL")
